...
function rotateCannonsVertically(angle){
	//keeping them symmetrical. For a convenient UI it takes angles in degrees as input. Note that the axis is relative to the cannon base's frame and not the world's frame, since the body is a child object of the base, so the axis to rotate is actually its x axis.
	...
	cannon1.body.rotation.x = 0.01 * angle;
	cannon2.body.rotation.x = 0.01 * angle;
...
function rotateCannonsHorizontally(angle){//keeping them symmetrical. An angle of zero makes both cannons face the front, so we would add 90 degrees
	...
	//or
	//cannon1.rotation.z=...
	cannon1.rotation.z = 0.05 *angle;
	cannon2.rotation.z = -(0.05 *angle);
	//cannon1.rotateOnAxis( new THREE.Vector3(0,0,1), 0.1)
	//cannon2.rotateOnAxis( new THREE.Vector3(0,0,-1), 0.1)
}
...
//You need to renderer.shadowMap.enabled = true; to enable shadows, 
//and add a light that supports casting shadows in the scene ,
// such as THREE.DirectionalLight (for a large scene you will need to set the frustrum of the shadow camera to be bigger - 
//see https://threejs.org/docs/#api/en/lights/shadows/DirectionalLightShadow), and set castShadow=true on objects that you want to cast shadows from, and set receiveShadow=true on objects you want to receive shadows such as the ground, and the receiving object must have a suppirting material such as THREE.MeshStandardMaterial or THREE.MeshLambertMaterial.

//*********************
//Firing cannons
//*********************
//we can create as many spheres as needed, but to save resources we can reuse spheres that need to be removed. 
//Also, to do physics, we need to have a list of the spheres in the scene. 
//Here's some code to manage and reuse spheres.
...
function fireCannon(cannon,material=sphereMaterial){
	...
	//to get the cannonball's starting position,
	// we can get the local position relative to the body's frame, and transform it into world frame.
	let startPosition=cannon.body.localToWorld(new THREE.Vector3(0,cannonballStartingLength,0));
	//localToWorld gets the world coordinates of the starting point in local coordinates. 
	//If the input is a Vector3, it assumes it's a point. If the input is Vector4, it treats it as a point or vector according to the fourth dimension.
	sphere.position.copy(startPosition);
	let startingVelocity=cannon.body.localToWorld(new THREE.Vector4(0,cannonballStartingLength,0,0));
	//now we want a vector, not a point.
	sphere.velocity=new THREE.Vector3();
	sphere.velocity.copy(startingVelocity).normalize().multiplyScalar(cannonballStartingSpeed);//copy xyz only
	//this is also a custom property added to the mesh object for convenience.
	// In more complex applications it's best to avoid the confusion that can be caused by adding random properties to library-defined objects.
	scene.add(sphere);
...
//Do physics
//*********************
//note: this is a simplified example of rigid-body physics code.
// Here we only consider linear velocity, not angular velocity or rotation, and only support spheres of the same size.
// You can use a physics library for better effects, or look up rigid body physics simulation for more information.
let G=10;
function physicsTick(dt){
	//Usually we should separate animation frames and physics ticks, 
	//so that we can pause physics, or adjust the time step size of physics, 
	//to keep physics running smoothly in real time, because the time between animation frames may not be constant.
	for(let i=0;i<sphereList.length;i++){
		let sphere=sphereList[i];
		//cannonballStartingSpeed
		//todo: physics!
		//todo 1. integrate velocity and gravity acceleration. 
		//in our case, velocity.z -= G*dt, position += velocity*dt 
		//sphere.velocity.z...
		//sphere.position.addScaledVector(...);
		//note: since the sphere is directly a child of the scene, both position
		// and velocity are in world frame, and we don't need to worry about transforming between frames here.
		//velocity: { }
		
		velx = sphere.velocity.x -= G*dt;
		vely = sphere.velocity.y -= G*dt;
		velz = sphere.velocity.z -= G*dt;
		sphere.position.x += velx * dt; 
		sphere.position.y += vely * dt;
		sphere.position.z += velz * dt; 
		
	//	sphere.position.addScaledVector(sphere.postition += sphere.velocity *dt, sphere.velocity * dt);

		//sphere.direction = cannon2.position.normalize();
		// =cannonballStartingSpeed * .25;
		//console.log(cannonballStartingSpeed)
		//sphere.velocity.addScaledVector( cannonballStartingSpeed * .01, dt );
		//sphere.position.addScaledVector(cannonballStartingSpeed * dt );
	/*	Vector3.copy( this.angularVelocity ).multiplyScalar( dt / 2 );
    quaternion.set( vector.x, vector.y, vector.z, 1 ).normalize();
    this.quaternion.multiply( quaternion );*/
	//	sphere.position.addScaledVector(2 * dt);
		...
		for(let j=0;j<i;j++){
			...
			if(sphere2.position.distanceTo(sphere.position) <= cannonballRadius*2 ){
				//1) simple case: if the cannons always fire symmetrically, 
				//cannonballs always collide symmetrically along the YZ plane, so we can just flip the x velocity value. 
				//sphere.velocity.x=...
				//sphere2.velocity.x=...
				sphere.velocity.x *=  -1;
				sphere2.velocity.x *=  -1;
				//sphere.position.x
				...
				//normal.copy(sphere2.position).addScaledVector(sphere.position,-1);
				//normal.normalize();
