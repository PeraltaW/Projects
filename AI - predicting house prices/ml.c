...
int main(int argc, char** argv){
  ...
  int colN,rowN, i, j;
    ...
    fscanf(fptr,"%d\n%d\n",&colN,&rowN);
    double** fmat=malloc(sizeof(double*)*(rowN*colN));
    for(i = 0 ; i< rowN; i++){
      fmat[i] = malloc(sizeof(double*)*(rowN*colN));}
     for(j= 0 ; j< colN; j++){
       fmat[j] = malloc(sizeof(double*)*(rowN*colN));}*/
/*   
    for(i = 0; i<rowN; i++){ if(feof(fptr)){break;}
      for(j=0; j<colN;j++){
	fscanf(fptr,"%lf%*c",&fmat[i][j]);
      }
    }
  ...
  double** xt= (double**)malloc((rowN*colN)*sizeof(double*));
  for(int o =0; o<rowN;o++){
    xt[o]=(double*)malloc((rowN*colN)*sizeof(double*));}
 xt = transposeMatrix(fmat, rowN , colN);

  double** matTran =malloc(sizeof(double*)*(rowN*colN));
    for(i = 0 ; i< rowN; i++){
    matTran[i] = malloc(sizeof(double*)*(rowN*colN));}*/
    
  /* matTran = transposeMatrix(fmat,rowN,colN);
  for(i = 0; i< rowN; i++){
      for(j=0; j<colN;j++){
...
double** transposeMatrix(double** mat, int row, int col)
{
  ...
  int i,j;
	...
	for(i = 0 ; i< row; i++){
      matTran[i] = malloc(sizeof(double*)*(row*col));}
