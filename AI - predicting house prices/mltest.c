...
weight = multiply(first, second, attributes,
		        attributes, attributes, 1);
double firstw = weight[0][0];
double** newweight = (double**)
             malloc(( attributes-1)*sizeof(double*));
...
//1st Iteration to Create Upper Diagonal Matrix
for(x=0; x< row; x++){
   for(y=0; y< col; y++){
     if(x==y){
       if(newMatrix[x][y] != 1){
  ...
  break;
   ...
   double element = newMatrix[x][y];
     for(z = 0; z< newcol; z++){
    newMatrix[x][z] =
      newMatrix[x][z] - (element* newMatrix[y][z]);
     }
   }
 }
}
//2nd Iteration to Create Bottom Diagonal Matrix
for(x=row-2; x>=0; x--){
  for(y=x+1; y<col; y++){
   if(newMatrix[x][y] != 0){
     double element = newMatrix[x][y];
